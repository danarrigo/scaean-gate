...
@Component({
  ...
  template: `
    <div class="app-shell">
      <header class="topbar"><div class="brand"><span class="crest">A</span><span><b>Apex</b><small>Trusted by Scaean Gate</small></span></div>
      ...
      <main>
        ...
        @else if (!authenticated) { <section class="empty-card"><span class="large-crest">A</span><p class="eyebrow">Apex workspace</p><h1>Enter with one identity</h1><p>Authenticate through Scaean Gate. Apex never receives or stores your password.</p><button class="primary" (click)="login()">Continue with Scaean Gate</button></section> }
        @else {
          <div class="dashboard">
            ...
            <section class="paper-card session"><div class="section-head"><div><p class="eyebrow">Local session</p><h2>Apex session</h2></div><span class="status active">● {{ session.status }}</span></div><dl><div><dt>Session ID</dt><dd><code>{{ session.id }}</code></dd></div><div><dt>Created</dt><dd>{{ session.createdAt | date:'medium' }}</dd></div><div><dt>Expires</dt><dd>{{ session.expiresAt | date:'medium' }}</dd></div></dl></section>
...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  localLogout(){if(!confirm('End only this Apex session?'))return;this.auth.localLogout().subscribe({next:()=>{this.authenticated=false;this.ended=false;this.say('Local session ended.');},error:e=>this.say(errorMessage(e))});}
